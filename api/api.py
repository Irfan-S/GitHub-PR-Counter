...
for count, u_name in enumerate(u_name_list,start=1):
    ...
    if(not json_obj.get("message")=='Validation Failed'):
        ...
        for item in json_obj.get("items"):
            pullreq_dict[u_name].append({"pr-number:":item.get("number"),"html_url":item.get("html_url"),"title":item.get("title"),"body":item.get("body"),"state":item.get("state")})
...
@app.route('/api/prs/users/all', methods=['GET'])
def api_all():
    return jsonify(pullreq_dict)

@app.route('/api/prs/users/top10',methods=["GET"])
def api_top10():
    sorted_dict = OrderedDict(sorted(pullreq_dict.items(), key = lambda x: len(x[1]), reverse=True))
    top10 = {}
    i=0
    ...
    top10 = dict(OrderedDict(sorted(top10.items(),key = lambda x: len(x[1]),reverse=True)))
    top10_list=[]
